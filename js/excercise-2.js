...
var sumOdd = function (inputArr) {
  return inputArr.reduce((acc, cur) => (cur % 2 === 0 ? acc : acc + cur) , 0);
};
...
var minInArr = function (inputArr) {
  return inputArr.reduce((min, cur) => (cur <= min ? cur : min) , 0);
};
...
var sumDigit = function (inputNum) {
  var stringVer = inputNum.toString();
  return stringVer.split('').reduce(((prev, cur) => prev += +cur), 0);
